...
const Transfer = () => {
  ...
  const { transferReceiver } = useSelector((state) => state.transactionReducer);

  useEffect(() => {
    dispatch(transactionActions.getDataReceiver());
  }, [dispatch]);

  const allUsers = useMemo(() => {
    if (!Array.isArray(transferReceiver)) return [];
  ...
  return (
      <div className="w-full pb-10">
        <div className="mb-6">
          ...
          <div>
            <Stepper steps={["Find People", "Set Nominal", "Finish"]} activeStep={0} />
          </div>
        </div>
              ...
              {filteredPeople.length > 0 ? (
                  filteredPeople.map((item, index) => (
                      ...
                      >
                        ...
                        <td className="hidden md:table-cell px-2 py-4 md:px-6 text-grey text-center">
                          {item.phone || "-"}
                        </td>
